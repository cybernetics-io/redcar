...
impl Client {
    pub fn new(c: &Config) -> Self {
        let channel = Channel::from_static(c.host).connect_lazy().unwrap();
        Client {
    ...
    pub async fn put(&mut self, key: Vec<u8>, value: Vec<u8>) {
        ...
        self.kv_client.put(req).await.unwrap();
    }

    pub async fn range(&mut self, key: Vec<u8>) -> Vec<Vec<u8>> {
        let req = Request::new(RangeRequest {
            key,
            limit: 0,
            sort_order: 0,
            sort_target: 0,
        ...
        let resp = self.kv_client.range(req).await.unwrap();
